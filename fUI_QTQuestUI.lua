...
function ns.FQTOptionsPanels.BuildQuest(ctx)
  ...
  local Quest = ns.Quest or {}
  local ResolveExpansionNameByID = Quest.ResolveExpansionNameByID or function(choices, id)
    id = tonumber(id)
  ...
  local NormalizeFontKey = Quest.NormalizeFontKey or function(key)
    key = tostring(key or "inherit")
    if key == "" then key = "inherit" end
    return key
  end
  local ResolveQuestColor = Quest.ResolveQuestColor or function(name)
    if name == "None" then
      return nil, "None"
  ...
  local function SetQuestColor(name)
    local c, normalized = ResolveQuestColor(name)
    pQuest._questColor = c
    pQuest._questColorName = normalized
    if UDDM_SetText then
      if normalized == "None" then
        UDDM_SetText(questColorDrop, "Color")
      elseif ColorLabel then
        UDDM_SetText(questColorDrop, ColorLabel(normalized))
      else
        UDDM_SetText(questColorDrop, normalized)
      end
    end
  end
