...
function ns.FQTOptionsPanels.BuildText(ctx)
  ...
  local Text = ns.Text or {}
  local ResolveExpansionNameByID = Text.ResolveExpansionNameByID or function(choices, id)
    id = tonumber(id)
  ...
  local ColorsMatch = Text.ColorsMatch or function(tbl, r, g, b)
    if type(tbl) ~= "table" then return false end
    return (tonumber(tbl[1]) == r) and (tonumber(tbl[2]) == g) and (tonumber(tbl[3]) == b)
  end
  local NormalizeFontKey = Text.NormalizeFontKey or function(key)
