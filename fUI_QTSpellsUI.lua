...
function ns.FQTOptionsPanels.BuildSpells(ctx)
  ...
  local Spells = ns.Spells or {}
  local SPELL_CLASS_TOKENS = Spells.SPELL_CLASS_TOKENS or {
    "DEATHKNIGHT","DEMONHUNTER","DRUID","EVOKER","HUNTER","MAGE","MONK","PALADIN","PRIEST","ROGUE","SHAMAN","WARLOCK","WARRIOR",
  }
  local WipeTable = Spells.WipeTable or function(t)
    if type(t) ~= "table" then return end
    for k in pairs(t) do
      t[k] = nil
    end
  end
  local ColorsMatch = Spells.ColorsMatch or function(tbl, r, g, b)
    if type(tbl) ~= "table" then return false end
    return (tonumber(tbl[1]) == r) and (tonumber(tbl[2]) == g) and (tonumber(tbl[3]) == b)
  end
  local ResolveExpansionNameByID = Spells.ResolveExpansionNameByID or function(choices, id)
    id = tonumber(id)
  ...
  function pSpells:_syncRuleCreateExpansion()
    ...
    if not name and id then
      local choices = (type(GetRuleExpansionChoices) == "function") and GetRuleExpansionChoices() or {}
      name = ResolveExpansionNameByID(choices, id)
    end
  ...
  local function GetSelectedSpellClasses()
    ...
    if type(Spells.GetSelectedSpellClasses) == "function" then
      return Spells.GetSelectedSpellClasses(panels.spells._classes)
    end
    local out = {}
    for _, tok in ipairs(SPELL_CLASS_TOKENS) do
      if panels.spells._classes[tok] then out[#out + 1] = tok end
    end
    return out
  end
  ...
  local function SetSelectedSpellClassesFromRule(value)
    ...
    if type(Spells.ApplySelectedSpellClassesFromRule) == "function" then
      local sel, single = Spells.ApplySelectedSpellClassesFromRule(panels.spells._classes, value)
      panels.spells._class = single
    else
  ...
  local function GetSpellNameSafeLocal(spellID)
    if type(Spells.GetSpellNameSafe) == "function" then
      return Spells.GetSpellNameSafe(spellID)
    end
  ...
  addSpellBtn:SetScript("OnClick", function()
    ...
    if not expName and expID then
      local choices = (type(GetRuleExpansionChoices) == "function") and GetRuleExpansionChoices() or {}
      expName = ResolveExpansionNameByID(choices, expID)
    end
